...
<div class="modal fade" id="deleteModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                Do you really want to delete <strong class="car_name"></strong>?
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
    ...
    <script>
        $(document).ready(function(){
            ...
            $(document).ready(function () {
            // Show car name in the delete modal
            $('.deleteBtn').on('click', function () {
                var car_name = $(this).data('name'); // Use .data() to retrieve data attributes
                var car_id = $(this).data('id'); // Get car ID
                $('.car_name').html(car_name); // Set car name in modal
                $('.deleteButton').data('id', car_id); // Set car ID on the delete button
                });
                
                // Perform delete operation
                $('.deleteButton').on('click', function () {
                    var car_id = $(this).data('id'); // Retrieve car ID from button
                    var url = "{{ route('deleteCar', 'car_id') }}"; // Laravel route
                    url = url.replace('car_id', car_id); // Replace placeholder with car ID
                    ...
                    type: 'GET',
                    beforeSend: function () {
                            $('.deleteButton').prop('disabled', true); // Disable button while processing
                        },
                    complete: function () {
                        $('.deleteButton').prop('disabled', false); // Enable button after processing
                    },
                    success: function (data) {
                        if (data.success) {
                            $('#deleteModal').modal('hide'); // Hide the modal
                            printSuccessMsg(data.msg); // Display success message
                            setTimeout(function () {
                                location.reload(true); // Reload the page
                            }, 2000);
                        } else {
                            printErrorMsg(data.msg); // Display error message
                            }
                        },
                    error: function (xhr, status, error) {
                        console.error('Error:', error); // Log error
                        printErrorMsg('Something went wrong!'); // Display generic error message
                        },
                    });
                });
                ...
                function printErrorMsg(msg) {
                    $('#alert-danger').html('').css('display', 'block').append(msg);
                }

                function printSuccessMsg(msg) {
                    $('#alert-success').html('').css('display', 'block').append(msg);
                }
            });
